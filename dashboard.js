...
const appSessionStatus = document.getElementById("app-session-status");
const heroPageLabel = document.getElementById("hero-page-label");
...
const heroProfileOverview = document.getElementById("hero-profile-overview");
const heroApplicationOverview = document.getElementById("hero-application-overview");
const heroTaskOverview = document.getElementById("hero-task-overview");
const heroDeadlineOverview = document.getElementById("hero-deadline-overview");
...
const pageMetadata = {
  scorer: {
    label: "Profile Scorer",
    title: "Build a clear picture of your admissions profile",
    description:
      "Translate your IGCSE or GCSE results, IB or A-Levels, testing, and activities into a structured view you can use across the rest of the admissions process."
  },
  questionnaire: {
    label: "Preferences",
    title: "Define the campus environment that fits your goals",
    description:
      "Capture financial aid needs, class size, climate, and campus priorities so GlobalAdmit can score fit more accurately in your planner."
  },
  planner: {
    label: "Application Planner",
    title: "Shape an ED, EA, and RD plan with stronger context",
    description:
      "Sort universities by fit or difficulty, keep round rules consistent, and save a cleaner admissions plan grounded in your profile and preferences."
  },
  statuses: {
    label: "Application Statuses",
    title: "Track every selected application from drafting to decision",
    description:
      "Keep each application outcome organized across rounds so your shortlist stays current as decisions arrive."
  },
  tasks: {
    label: "Task Manager",
    title: "Turn the admissions plan into concrete next actions",
    description:
      "Prioritize essays, testing, financial aid logistics, and other tasks so the admissions process stays manageable and on schedule."
  }
};

const pageRegistry = [
  { name: "scorer", section: pageScorer, button: navScorer },
  { name: "questionnaire", section: pageQuestionnaire, button: navQuestionnaire },
  { name: "planner", section: pagePlanner, button: navPlanner },
  { name: "statuses", section: pageStatuses, button: navStatuses },
  { name: "tasks", section: pageTasks, button: navTasks }
];
...
function updateHeroContent(pageName) {
  const metadata = pageMetadata[pageName] || pageMetadata.scorer;

  if (heroPageLabel) {
    heroPageLabel.textContent = metadata.label;
  }
  if (heroPageTitle) {
    heroPageTitle.textContent = metadata.title;
  }
  if (heroPageDescription) {
    heroPageDescription.textContent = metadata.description;
  }

  document.title = `GlobalAdmit | ${metadata.label}`;
}

function updateOverviewMetrics() {
  if (heroProfileOverview) {
    heroProfileOverview.textContent =
      currentProfileScore === null ? "Awaiting calculation" : currentProfileScore.toFixed(1);
  }

  const selectedApplications = getAppliedSchoolSelections();
  if (heroApplicationOverview) {
    heroApplicationOverview.textContent =
      selectedApplications.length === 0 ? "0 selected" : `${selectedApplications.length} selected`;
  }
  ...
  if (heroTaskOverview) {
    if (tasks.length === 0) {
      heroTaskOverview.textContent = "No tasks";
    } else if (completedTasks === 0) {
      heroTaskOverview.textContent = `${openTasks} open`;
    } else {
      heroTaskOverview.textContent = `${openTasks} open / ${completedTasks} done`;
    }
  }

  if (heroDeadlineOverview) {
    if (selectedApplications.length === 0) {
      heroDeadlineOverview.textContent = "Not set";
    } else {
      ...
      heroDeadlineOverview.textContent = `${earliestApplication.round} · ${earliestApplication.deadline}`;
    }
  }
}
...
function resetDerivedState() {
  ...
  profileMessage.textContent = "Enter your academic record and activities to calculate your GlobalAdmit profile.";
...
function applyLoadedProfile(profile) {
  ...
  if (currentProfileScore !== null && currentAcademicScore !== null) {
    ...
    profileMessage.textContent = "Saved profile loaded. Recalculate after edits to refresh your score.";
    scoreBreakdown.hidden = false;
    scoreBreakdown.innerHTML = `
      <div class="section-heading section-heading-compact">
        <span class="section-number">10</span>
        <div>
          <h2>Saved Score Snapshot</h2>
          <p class="page-intro">This saved score is loaded from your last session. Recalculate after making updates.</p>
        </div>
      </div>
...
function showPage(pageName) {
  const activePage = pageMetadata[pageName] ? pageName : "scorer";

  pageRegistry.forEach(({ name, section, button }) => {
...
async function calculateAndRender() {
  ...
  profileScoreValue.textContent = currentProfileScore.toFixed(1);
  profileMessage.textContent = "ProfileScore = 0.67 x Overall Academic Score + 0.33 x Extracurricular Score";
  scoreBreakdown.hidden = false;
  scoreBreakdown.innerHTML = `
    <div class="section-heading section-heading-compact">
      <span class="section-number">10</span>
      <div>
        ...
        <p class="page-intro">Review how each academic and extracurricular input contributes to the final score.</p>
      </div>
    </div>
    <div class="breakdown-grid">
      <div class="breakdown-item">
        ...
        <article class="status-card">
          <div class="status-card-main">
            <div class="status-card-topline">
              ...
              <span class="badge status-round-badge">${application.round}</span>
            </div>
            <p class="helper-text">Deadline ${application.deadline}</p>
          </div>
...
function renderApplicationStatuses() {
  ...
  renderApplicationStatusSection(statusesEdList, edApplications, "No Early Decision universities are in your plan yet.");
  renderApplicationStatusSection(statusesEaList, eaApplications, "No Early Action universities are in your plan yet.");
  renderApplicationStatusSection(statusesRdList, rdApplications, "No Regular Decision universities are in your plan yet.");
}
      ...
      (school) => `
        <label class="planner-option ${getPlannerAvailability(school).disabled ? "planner-option-disabled" : ""}" data-school="${school.name}">
          ...
          <span class="planner-option-text">
            <span class="planner-option-heading">
              <strong>${school.name}</strong>
              <span class="planner-deadline">${school.ed_deadline ? `Deadline ${school.ed_deadline}` : "Deadline unavailable"}</span>
            </span>
            <span class="planner-option-meta">
              <span class="planner-option-round">Binding Early Decision</span>
      ...
      (school) => `
        <label class="planner-option ${getPlannerAvailability(school).disabled ? "planner-option-disabled" : ""}" data-school="${school.name}">
          ...
          <span class="planner-option-text">
            <span class="planner-option-heading">
              <strong>${school.name}</strong>
              <span class="planner-deadline">${school.ea_deadline ? `Deadline ${school.ea_deadline}` : "Deadline unavailable"}</span>
            </span>
            <span class="planner-option-meta">
              <span class="planner-option-round">Non-binding Early Action</span>
              ${getPlannerBadgesMarkup(school.name)}
              ${getPlannerRestrictionMarkup(school)}
            </span>
          </span>
      ...
      (school) => `
        <label class="planner-option ${getPlannerAvailability(school).disabled ? "planner-option-disabled" : ""}" data-school="${school.name}">
          ...
          <span class="planner-option-text">
            <span class="planner-option-heading">
              <strong>${school.name}</strong>
              <span class="planner-deadline">${school.rd_deadline ? `Deadline ${school.rd_deadline}` : "Deadline unavailable"}</span>
            </span>
            <span class="planner-option-meta">
              <span class="planner-option-round">Regular Decision</span>
              ${getPlannerBadgesMarkup(school.name)}
              ${getPlannerRestrictionMarkup(school)}
            </span>
          </span>
...
function renderQuestionnaireSummary() {
  ...
  if (filledValues.length === 0) {
    ...
    questionnaireSummary.textContent = "Complete your preferences to start generating fit context in the planner.";
    return;
  }
    ...
    .map(
      (item, index) => `
        <div class="ranking-item" draggable="true" data-campus="${item}">
          <span class="ranking-handle">Drag</span>
...
function getTaskStateClass(task) {
  if (task.completed) return "task-chip-status-completed";
  if (isTaskOverdue(task)) return "task-chip-status-overdue";
  return "task-chip-status-upcoming";
}
...
function renderTaskList() {
  ...
  if (visibleTasks.length === 0) {
    ...
    taskList.textContent = "No tasks match this view yet.";
    updateOverviewMetrics();
    return;
  }
  ...
  <div class="task-card-top">
    <div class="task-main">
      <div class="task-title-row">
        <div>
          <h3>${task.title}</h3>
          <p>${task.type} · Due ${formatTaskDate(task.date)} · ${task.hours} hour${task.hours === 1 ? "" : "s"}</p>
        </div>
        <div class="task-meta">
          <span class="task-chip ${getPriorityClass(task.priority)}">Priority ${task.priority}</span>
          <span class="task-chip ${getTaskStateClass(task)}">${getTaskStateLabel(task)}</span>
          ${task.autogenerated ? '<span class="task-chip task-chip-auto">Auto-created</span>' : ""}
        </div>
      </div>
      <div class="task-actions">
        <button type="button" class="secondary-button task-edit-button" data-task-id="${task.id}">Edit</button>
        <button type="button" class="secondary-button task-toggle-button" data-task-id="${task.id}">
          ${task.completed ? "Mark upcoming" : "Mark completed"}
        </button>
      </div>
    </div>
    <div class="task-side">
      <button type="button" class="secondary-button task-remove-button" data-task-id="${task.id}">Remove task</button>
    </div>
  </div>
</article>
      `
